#' Google Analytics API Objects 
#' Views and manages your Google Analytics data.
#' 
#' Auto-generated code by googleAuthR::gar_create_api_objects
#'  at 2016-08-01 23:28:19
#' filename: ./inst/api_objects.R
#' api_json: gar_discovery_api analytics v3
#' 
#' Objects for use by the functions created by googleAuthR::gar_create_api_skeleton

#' Account Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics account entry.
#' 
#' @param Account.childLink The \link{Account.childLink} object or list of objects
#' @param Account.permissions The \link{Account.permissions} object or list of objects
#' @param childLink Child link for an account entry
#' @param created Time the account was created
#' @param id Account ID
#' @param name Account name
#' @param permissions Permissions the user has for this account
#' @param selfLink Link for this account
#' @param starred Indicates whether this account is starred or not
#' @param updated Time the account was last modified
#' 
#' @return Account object
#' 
#' @family Account functions
#' @export
Account <- function(Account.childLink = NULL, Account.permissions = NULL, childLink = NULL, 
    created = NULL, id = NULL, name = NULL, permissions = NULL, selfLink = NULL, 
    starred = NULL, updated = NULL) {
    structure(list(Account.childLink = Account.childLink, Account.permissions = Account.permissions, 
        childLink = childLink, created = created, id = id, kind = "analytics#account", 
        name = name, permissions = permissions, selfLink = selfLink, starred = starred, 
        updated = updated), class = "gar_Account")
}

#' Account.childLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Child link for an account entry. Points to the list of web properties for this account.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the list of web properties for this account
#' 
#' @return Account.childLink object
#' 
#' @family Account functions
#' @export
Account.childLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#webproperties"), class = "gar_Account.childLink")
}

#' Account.permissions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Permissions the user has for this account.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Account.permissions object
#' 
#' @family Account functions
#' @export
Account.permissions <- function() {
    list()
}

#' AccountRef Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a linked account.
#' 
#' @param href The \link{} object or list of objects
#' @param id Link for this account
#' @param name Account ID
#' @param href Account name
#' 
#' @return AccountRef object
#' 
#' @family AccountRef functions
#' @export
AccountRef <- function(href = NULL, id = NULL, name = NULL) {
    structure(list(href = href, id = id, kind = "analytics#accountRef", name = name), 
        class = "gar_AccountRef")
}

#' AccountSummaries Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An AccountSummary collection lists a summary of accounts, properties and views (profiles) to which the user has access. Each resource in the collection corresponds to a single AccountSummary.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of AccountSummaries
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this AccountSummary collection
#' @param startIndex Link to previous page for this AccountSummary collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return AccountSummaries object
#' 
#' @family AccountSummaries functions
#' @export
AccountSummaries <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#accountSummaries", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_AccountSummaries")
}

#' AccountSummary Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics AccountSummary. An AccountSummary is a lightweight tree comprised of properties/profiles.
#' 
#' @param id The \link{} object or list of objects
#' @param name Account ID
#' @param starred Account name
#' @param webProperties Indicates whether this account is starred or not
#' @param id List of web properties under this account
#' 
#' @return AccountSummary object
#' 
#' @family AccountSummary functions
#' @export
AccountSummary <- function(id = NULL, name = NULL, starred = NULL, webProperties = NULL) {
    structure(list(id = id, kind = "analytics#accountSummary", name = name, starred = starred, 
        webProperties = webProperties), class = "gar_AccountSummary")
}

#' AccountTicket Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics account ticket. The account ticket consists of the ticket ID and the basic information for the account, property and profile.
#' 
#' @param account The \link{} object or list of objects
#' @param id Account for this ticket
#' @param profile Account ticket ID used to access the account ticket
#' @param redirectUri View (Profile) for the account
#' @param webproperty Redirect URI where the user will be sent after accepting Terms of Service
#' @param account Web property for the account
#' 
#' @return AccountTicket object
#' 
#' @family AccountTicket functions
#' @export
AccountTicket <- function(account = NULL, id = NULL, profile = NULL, redirectUri = NULL, 
    webproperty = NULL) {
    structure(list(account = account, id = id, kind = "analytics#accountTicket", 
        profile = profile, redirectUri = redirectUri, webproperty = webproperty), 
        class = "gar_AccountTicket")
}

#' Accounts Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An account collection provides a list of Analytics accounts to which a user has access. The account collection is the entry point to all management information. Each resource in the collection corresponds to a single Analytics account.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of accounts
#' @param nextLink The maximum number of entries the response can contain, regardless of the actual number of entries returned
#' @param previousLink Next link for this account collection
#' @param startIndex Previous link for this account collection
#' @param totalResults The starting index of the entries, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return Accounts object
#' 
#' @family Accounts functions
#' @export
Accounts <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#accounts", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Accounts")
}

#' AdWordsAccount Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an AdWords account.
#' 
#' @param autoTaggingEnabled The \link{} object or list of objects
#' @param customerId True if auto-tagging is enabled on the AdWords account
#' @param autoTaggingEnabled Customer ID
#' 
#' @return AdWordsAccount object
#' 
#' @family AdWordsAccount functions
#' @export
AdWordsAccount <- function(autoTaggingEnabled = NULL, customerId = NULL) {
    structure(list(autoTaggingEnabled = autoTaggingEnabled, customerId = customerId, 
        kind = "analytics#adWordsAccount"), class = "gar_AdWordsAccount")
}

#' AnalyticsDataimportDeleteUploadDataRequest Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Request template for the delete upload data request.
#' 
#' @param customDataImportUids The \link{} object or list of objects
#' @param customDataImportUids A list of upload UIDs
#' 
#' @return AnalyticsDataimportDeleteUploadDataRequest object
#' 
#' @family AnalyticsDataimportDeleteUploadDataRequest functions
#' @export
AnalyticsDataimportDeleteUploadDataRequest <- function(customDataImportUids = NULL) {
    structure(list(customDataImportUids = customDataImportUids), class = "gar_AnalyticsDataimportDeleteUploadDataRequest")
}

#' Column Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a metadata column.
#' 
#' @param Column.attributes The \link{Column.attributes} object or list of objects
#' @param attributes Map of attribute name and value for this column
#' @param id Column id
#' 
#' @return Column object
#' 
#' @family Column functions
#' @export
Column <- function(Column.attributes = NULL, attributes = NULL, id = NULL) {
    structure(list(Column.attributes = Column.attributes, attributes = attributes, 
        id = id, kind = "analytics#column"), class = "gar_Column")
}

#' Column.attributes Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Map of attribute name and value for this column.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Column.attributes object
#' 
#' @family Column functions
#' @export
Column.attributes <- function() {
    list()
}

#' Columns Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Lists columns (dimensions and metrics) for a particular report type.
#' 
#' @param attributeNames The \link{} object or list of objects
#' @param etag List of attributes names returned by columns
#' @param items Etag of collection
#' @param totalResults List of columns for a report type
#' @param attributeNames Total number of columns returned in the response
#' 
#' @return Columns object
#' 
#' @family Columns functions
#' @export
Columns <- function(attributeNames = NULL, etag = NULL, items = NULL, totalResults = NULL) {
    structure(list(attributeNames = attributeNames, etag = etag, items = items, kind = "analytics#columns", 
        totalResults = totalResults), class = "gar_Columns")
}

#' CustomDataSource Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics custom data source.
#' 
#' @param CustomDataSource.childLink The \link{CustomDataSource.childLink} object or list of objects
#' @param CustomDataSource.parentLink The \link{CustomDataSource.parentLink} object or list of objects
#' @param accountId Account ID to which this custom data source belongs
#' @param childLink 
#' @param created Time this custom data source was created
#' @param description Description of custom data source
#' @param id Custom data source ID
#' @param importBehavior 
#' @param name Name of this custom data source
#' @param parentLink Parent link for this custom data source
#' @param profilesLinked IDs of views (profiles) linked to the custom data source
#' @param selfLink Link for this Analytics custom data source
#' @param updated Time this custom data source was last modified
#' @param uploadType 
#' @param webPropertyId Web property ID of the form UA-XXXXX-YY to which this custom data source belongs
#' 
#' @return CustomDataSource object
#' 
#' @family CustomDataSource functions
#' @export
CustomDataSource <- function(CustomDataSource.childLink = NULL, CustomDataSource.parentLink = NULL, 
    accountId = NULL, childLink = NULL, created = NULL, description = NULL, id = NULL, 
    importBehavior = NULL, name = NULL, parentLink = NULL, profilesLinked = NULL, 
    selfLink = NULL, updated = NULL, uploadType = NULL, webPropertyId = NULL) {
    structure(list(CustomDataSource.childLink = CustomDataSource.childLink, CustomDataSource.parentLink = CustomDataSource.parentLink, 
        accountId = accountId, childLink = childLink, created = created, description = description, 
        id = id, importBehavior = importBehavior, kind = "analytics#customDataSource", 
        name = name, parentLink = parentLink, profilesLinked = profilesLinked, selfLink = selfLink, 
        type = type, updated = updated, uploadType = uploadType, webPropertyId = webPropertyId), 
        class = "gar_CustomDataSource")
}

#' CustomDataSource.childLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the list of daily uploads for this custom data source
#' 
#' @return CustomDataSource.childLink object
#' 
#' @family CustomDataSource functions
#' @export
CustomDataSource.childLink <- function(href = NULL) {
    structure(list(href = href, type = type), class = "gar_CustomDataSource.childLink")
}

#' CustomDataSource.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for this custom data source. Points to the web property to which this custom data source belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the web property to which this custom data source belongs
#' 
#' @return CustomDataSource.parentLink object
#' 
#' @family CustomDataSource functions
#' @export
CustomDataSource.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#webproperty"), class = "gar_CustomDataSource.parentLink")
}

#' CustomDataSources Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Lists Analytics custom data sources to which the user has access. Each resource in the collection corresponds to a single Analytics custom data source.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage Collection of custom data sources
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this custom data source collection
#' @param startIndex Link to previous page for this custom data source collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return CustomDataSources object
#' 
#' @family CustomDataSources functions
#' @export
CustomDataSources <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#customDataSources", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_CustomDataSources")
}

#' CustomDimension Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics Custom Dimension.
#' 
#' @param CustomDimension.parentLink The \link{CustomDimension.parentLink} object or list of objects
#' @param accountId Account ID
#' @param active Boolean indicating whether the custom dimension is active
#' @param id Custom dimension ID
#' @param name Name of the custom dimension
#' @param parentLink Parent link for the custom dimension
#' @param scope Scope of the custom dimension: HIT, SESSION, USER or PRODUCT
#' @param webPropertyId Property ID
#' 
#' @return CustomDimension object
#' 
#' @family CustomDimension functions
#' @export
CustomDimension <- function(CustomDimension.parentLink = NULL, accountId = NULL, 
    active = NULL, id = NULL, name = NULL, parentLink = NULL, scope = NULL, webPropertyId = NULL) {
    structure(list(CustomDimension.parentLink = CustomDimension.parentLink, accountId = accountId, 
        active = active, id = id, name = name, parentLink = parentLink, scope = scope, 
        webPropertyId = webPropertyId), class = "gar_CustomDimension")
}

#' CustomDimension.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for the custom dimension. Points to the property to which the custom dimension belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the property to which the custom dimension belongs
#' 
#' @return CustomDimension.parentLink object
#' 
#' @family CustomDimension functions
#' @export
CustomDimension.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#webproperty"), class = "gar_CustomDimension.parentLink")
}

#' CustomDimensions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A custom dimension collection lists Analytics custom dimensions to which the user has access. Each resource in the collection corresponds to a single Analytics custom dimension.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage Collection of custom dimensions
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this custom dimension collection
#' @param startIndex Link to previous page for this custom dimension collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return CustomDimensions object
#' 
#' @family CustomDimensions functions
#' @export
CustomDimensions <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#customDimensions", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_CustomDimensions")
}

#' CustomMetric Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics Custom Metric.
#' 
#' @param CustomMetric.parentLink The \link{CustomMetric.parentLink} object or list of objects
#' @param accountId Account ID
#' @param active Boolean indicating whether the custom metric is active
#' @param id Custom metric ID
#' @param max_value Max value of custom metric
#' @param min_value Min value of custom metric
#' @param name Name of the custom metric
#' @param parentLink Parent link for the custom metric
#' @param scope Scope of the custom metric: HIT or PRODUCT
#' @param webPropertyId Property ID
#' 
#' @return CustomMetric object
#' 
#' @family CustomMetric functions
#' @export
CustomMetric <- function(CustomMetric.parentLink = NULL, accountId = NULL, active = NULL, 
    id = NULL, max_value = NULL, min_value = NULL, name = NULL, parentLink = NULL, 
    scope = NULL, webPropertyId = NULL) {
    structure(list(CustomMetric.parentLink = CustomMetric.parentLink, accountId = accountId, 
        active = active, id = id, max_value = max_value, min_value = min_value, name = name, 
        parentLink = parentLink, scope = scope, type = type, webPropertyId = webPropertyId), 
        class = "gar_CustomMetric")
}

#' CustomMetric.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for the custom metric. Points to the property to which the custom metric belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the property to which the custom metric belongs
#' 
#' @return CustomMetric.parentLink object
#' 
#' @family CustomMetric functions
#' @export
CustomMetric.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#webproperty"), class = "gar_CustomMetric.parentLink")
}

#' CustomMetrics Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A custom metric collection lists Analytics custom metrics to which the user has access. Each resource in the collection corresponds to a single Analytics custom metric.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage Collection of custom metrics
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this custom metric collection
#' @param startIndex Link to previous page for this custom metric collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return CustomMetrics object
#' 
#' @family CustomMetrics functions
#' @export
CustomMetrics <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#customMetrics", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_CustomMetrics")
}

#' EntityAdWordsLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics Entity AdWords Link.
#' 
#' @param EntityAdWordsLink.entity The \link{EntityAdWordsLink.entity} object or list of objects
#' @param adWordsAccounts A list of AdWords client accounts
#' @param entity Web property being linked
#' @param id Entity AdWords link ID
#' @param name Name of the link
#' @param profileIds IDs of linked Views (Profiles) represented as strings
#' @param selfLink URL link for this Google Analytics - Google AdWords link
#' 
#' @return EntityAdWordsLink object
#' 
#' @family EntityAdWordsLink functions
#' @export
EntityAdWordsLink <- function(EntityAdWordsLink.entity = NULL, adWordsAccounts = NULL, 
    entity = NULL, id = NULL, name = NULL, profileIds = NULL, selfLink = NULL) {
    structure(list(EntityAdWordsLink.entity = EntityAdWordsLink.entity, adWordsAccounts = adWordsAccounts, 
        entity = entity, id = id, kind = "analytics#entityAdWordsLink", name = name, 
        profileIds = profileIds, selfLink = selfLink), class = "gar_EntityAdWordsLink")
}

#' EntityAdWordsLink.entity Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Web property being linked.
#' 
#' @param webPropertyRef The \link{} object or list of objects
#' 
#' @return EntityAdWordsLink.entity object
#' 
#' @family EntityAdWordsLink functions
#' @export
EntityAdWordsLink.entity <- function(webPropertyRef = NULL) {
    structure(list(webPropertyRef = webPropertyRef), class = "gar_EntityAdWordsLink.entity")
}

#' EntityAdWordsLinks Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An entity AdWords link collection provides a list of GA-AdWords links Each resource in this collection corresponds to a single link.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of entity AdWords links
#' @param nextLink The maximum number of entries the response can contain, regardless of the actual number of entries returned
#' @param previousLink Next link for this AdWords link collection
#' @param startIndex Previous link for this AdWords link collection
#' @param totalResults The starting index of the entries, which is 1 by default or otherwise specified by the start-index query parameter
#' @param items The total number of results for the query, regardless of the number of results in the response
#' 
#' @return EntityAdWordsLinks object
#' 
#' @family EntityAdWordsLinks functions
#' @export
EntityAdWordsLinks <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#entityAdWordsLinks", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults), class = "gar_EntityAdWordsLinks")
}

#' EntityUserLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics Entity-User Link. Returns permissions that a user has for an entity.
#' 
#' @param EntityUserLink.entity The \link{EntityUserLink.entity} object or list of objects
#' @param EntityUserLink.permissions The \link{EntityUserLink.permissions} object or list of objects
#' @param entity Entity for this link
#' @param id Entity user link ID
#' @param permissions Permissions the user has for this entity
#' @param selfLink Self link for this resource
#' @param userRef User reference
#' 
#' @return EntityUserLink object
#' 
#' @family EntityUserLink functions
#' @export
EntityUserLink <- function(EntityUserLink.entity = NULL, EntityUserLink.permissions = NULL, 
    entity = NULL, id = NULL, permissions = NULL, selfLink = NULL, userRef = NULL) {
    structure(list(EntityUserLink.entity = EntityUserLink.entity, EntityUserLink.permissions = EntityUserLink.permissions, 
        entity = entity, id = id, kind = "analytics#entityUserLink", permissions = permissions, 
        selfLink = selfLink, userRef = userRef), class = "gar_EntityUserLink")
}

#' EntityUserLink.entity Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Entity for this link. It can be an account, a web property, or a view (profile).
#' 
#' @param accountRef The \link{} object or list of objects
#' @param profileRef Account for this link
#' @param webPropertyRef View (Profile) for this link
#' @param accountRef Web property for this link
#' 
#' @return EntityUserLink.entity object
#' 
#' @family EntityUserLink functions
#' @export
EntityUserLink.entity <- function(accountRef = NULL, profileRef = NULL, webPropertyRef = NULL) {
    structure(list(accountRef = accountRef, profileRef = profileRef, webPropertyRef = webPropertyRef), 
        class = "gar_EntityUserLink.entity")
}

#' EntityUserLink.permissions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Permissions the user has for this entity.
#' 
#' @param local The \link{} object or list of objects
#' @param local Permissions that a user has been assigned at this very level
#' 
#' @return EntityUserLink.permissions object
#' 
#' @family EntityUserLink functions
#' @export
EntityUserLink.permissions <- function(local = NULL) {
    structure(list(local = local), class = "gar_EntityUserLink.permissions")
}

#' EntityUserLinks Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An entity user link collection provides a list of Analytics ACL links Each resource in this collection corresponds to a single link.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of entity user links
#' @param nextLink The maximum number of entries the response can contain, regardless of the actual number of entries returned
#' @param previousLink Next link for this account collection
#' @param startIndex Previous link for this account collection
#' @param totalResults The starting index of the entries, which is 1 by default or otherwise specified by the start-index query parameter
#' @param items The total number of results for the query, regardless of the number of results in the response
#' 
#' @return EntityUserLinks object
#' 
#' @family EntityUserLinks functions
#' @export
EntityUserLinks <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#entityUserLinks", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults), class = "gar_EntityUserLinks")
}

#' Experiment Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics experiment resource.
#' 
#' @param Experiment.parentLink The \link{Experiment.parentLink} object or list of objects
#' @param Experiment.variations The \link{Experiment.variations} object or list of objects
#' @param accountId Account ID to which this experiment belongs
#' @param created Time the experiment was created
#' @param description Notes about this experiment
#' @param editableInGaUi If true, the end user will be able to edit the experiment via the Google Analytics user interface
#' @param endTime The ending time of the experiment (the time the status changed from RUNNING to ENDED)
#' @param equalWeighting Boolean specifying whether to distribute traffic evenly across all variations
#' @param id Experiment ID
#' @param internalWebPropertyId Internal ID for the web property to which this experiment belongs
#' @param minimumExperimentLengthInDays An integer number in [3, 90]
#' @param name Experiment name
#' @param objectiveMetric The metric that the experiment is optimizing
#' @param optimizationType Whether the objectiveMetric should be minimized or maximized
#' @param parentLink Parent link for an experiment
#' @param profileId View (Profile) ID to which this experiment belongs
#' @param reasonExperimentEnded Why the experiment ended
#' @param rewriteVariationUrlsAsOriginal Boolean specifying whether variations URLS are rewritten to match those of the original
#' @param selfLink Link for this experiment
#' @param servingFramework The framework used to serve the experiment variations and evaluate the results
#' @param snippet The snippet of code to include on the control page(s)
#' @param startTime The starting time of the experiment (the time the status changed from READY_TO_RUN to RUNNING)
#' @param status Experiment status
#' @param trafficCoverage A floating-point number in (0, 1]
#' @param updated Time the experiment was last modified
#' @param variations Array of variations
#' @param webPropertyId Web property ID to which this experiment belongs
#' @param winnerConfidenceLevel A floating-point number in (0, 1)
#' @param winnerFound Boolean specifying whether a winner has been found for this experiment
#' 
#' @return Experiment object
#' 
#' @family Experiment functions
#' @export
Experiment <- function(Experiment.parentLink = NULL, Experiment.variations = NULL, 
    accountId = NULL, created = NULL, description = NULL, editableInGaUi = NULL, 
    endTime = NULL, equalWeighting = NULL, id = NULL, internalWebPropertyId = NULL, 
    minimumExperimentLengthInDays = NULL, name = NULL, objectiveMetric = NULL, optimizationType = NULL, 
    parentLink = NULL, profileId = NULL, reasonExperimentEnded = NULL, rewriteVariationUrlsAsOriginal = NULL, 
    selfLink = NULL, servingFramework = NULL, snippet = NULL, startTime = NULL, status = NULL, 
    trafficCoverage = NULL, updated = NULL, variations = NULL, webPropertyId = NULL, 
    winnerConfidenceLevel = NULL, winnerFound = NULL) {
    structure(list(Experiment.parentLink = Experiment.parentLink, Experiment.variations = Experiment.variations, 
        accountId = accountId, created = created, description = description, editableInGaUi = editableInGaUi, 
        endTime = endTime, equalWeighting = equalWeighting, id = id, internalWebPropertyId = internalWebPropertyId, 
        kind = "analytics#experiment", minimumExperimentLengthInDays = minimumExperimentLengthInDays, 
        name = name, objectiveMetric = objectiveMetric, optimizationType = optimizationType, 
        parentLink = parentLink, profileId = profileId, reasonExperimentEnded = reasonExperimentEnded, 
        rewriteVariationUrlsAsOriginal = rewriteVariationUrlsAsOriginal, selfLink = selfLink, 
        servingFramework = servingFramework, snippet = snippet, startTime = startTime, 
        status = status, trafficCoverage = trafficCoverage, updated = updated, variations = variations, 
        webPropertyId = webPropertyId, winnerConfidenceLevel = winnerConfidenceLevel, 
        winnerFound = winnerFound), class = "gar_Experiment")
}

#' Experiment.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for an experiment. Points to the view (profile) to which this experiment belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the view (profile) to which this experiment belongs
#' 
#' @return Experiment.parentLink object
#' 
#' @family Experiment functions
#' @export
Experiment.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#profile"), class = "gar_Experiment.parentLink")
}

#' Experiment.variations Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Experiment.variations object
#' 
#' @family Experiment functions
#' @export
Experiment.variations <- function() {
    list()
}

#' Experiments Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An experiment collection lists Analytics experiments to which the user has access. Each view (profile) can have a set of experiments. Each resource in the Experiment collection corresponds to a single Analytics experiment.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of experiments
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this experiment collection
#' @param startIndex Link to previous page for this experiment collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of resources in the result
#' @param items Email ID of the authenticated user
#' 
#' @return Experiments object
#' 
#' @family Experiments functions
#' @export
Experiments <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#experiments", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Experiments")
}

#' Filter Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics account filter.
#' 
#' @param Filter.advancedDetails The \link{Filter.advancedDetails} object or list of objects
#' @param Filter.lowercaseDetails The \link{Filter.lowercaseDetails} object or list of objects
#' @param Filter.parentLink The \link{Filter.parentLink} object or list of objects
#' @param Filter.searchAndReplaceDetails The \link{Filter.searchAndReplaceDetails} object or list of objects
#' @param Filter.uppercaseDetails The \link{Filter.uppercaseDetails} object or list of objects
#' @param accountId Account ID to which this filter belongs
#' @param advancedDetails Details for the filter of the type ADVANCED
#' @param excludeDetails Details for the filter of the type EXCLUDE
#' @param id Filter ID
#' @param includeDetails Details for the filter of the type INCLUDE
#' @param lowercaseDetails Details for the filter of the type LOWER
#' @param name Name of this filter
#' @param parentLink Parent link for this filter
#' @param searchAndReplaceDetails Details for the filter of the type SEARCH_AND_REPLACE
#' @param uppercaseDetails Details for the filter of the type UPPER
#' 
#' @return Filter object
#' 
#' @family Filter functions
#' @export
Filter <- function(Filter.advancedDetails = NULL, Filter.lowercaseDetails = NULL, 
    Filter.parentLink = NULL, Filter.searchAndReplaceDetails = NULL, Filter.uppercaseDetails = NULL, 
    accountId = NULL, advancedDetails = NULL, excludeDetails = NULL, id = NULL, includeDetails = NULL, 
    lowercaseDetails = NULL, name = NULL, parentLink = NULL, searchAndReplaceDetails = NULL, 
    uppercaseDetails = NULL) {
    structure(list(Filter.advancedDetails = Filter.advancedDetails, Filter.lowercaseDetails = Filter.lowercaseDetails, 
        Filter.parentLink = Filter.parentLink, Filter.searchAndReplaceDetails = Filter.searchAndReplaceDetails, 
        Filter.uppercaseDetails = Filter.uppercaseDetails, accountId = accountId, 
        advancedDetails = advancedDetails, excludeDetails = excludeDetails, id = id, 
        includeDetails = includeDetails, lowercaseDetails = lowercaseDetails, name = name, 
        parentLink = parentLink, searchAndReplaceDetails = searchAndReplaceDetails, 
        type = type, uppercaseDetails = uppercaseDetails), class = "gar_Filter")
}

#' Filter.advancedDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the filter of the type ADVANCED.
#' 
#' @param caseSensitive The \link{} object or list of objects
#' @param extractA Indicates if the filter expressions are case sensitive
#' @param extractB Expression to extract from field A
#' @param fieldA Expression to extract from field B
#' @param fieldAIndex Field A
#' @param fieldARequired The Index of the custom dimension
#' @param fieldB Indicates if field A is required to match
#' @param fieldBIndex Field B
#' @param fieldBRequired The Index of the custom dimension
#' @param outputConstructor Indicates if field B is required to match
#' @param outputToField Expression used to construct the output value
#' @param outputToFieldIndex Output field
#' @param overrideOutputField The Index of the custom dimension
#' @param caseSensitive Indicates if the existing value of the output field, if any, should be overridden by the output expression
#' 
#' @return Filter.advancedDetails object
#' 
#' @family Filter functions
#' @export
Filter.advancedDetails <- function(caseSensitive = NULL, extractA = NULL, extractB = NULL, 
    fieldA = NULL, fieldAIndex = NULL, fieldARequired = NULL, fieldB = NULL, fieldBIndex = NULL, 
    fieldBRequired = NULL, outputConstructor = NULL, outputToField = NULL, outputToFieldIndex = NULL, 
    overrideOutputField = NULL) {
    structure(list(caseSensitive = caseSensitive, extractA = extractA, extractB = extractB, 
        fieldA = fieldA, fieldAIndex = fieldAIndex, fieldARequired = fieldARequired, 
        fieldB = fieldB, fieldBIndex = fieldBIndex, fieldBRequired = fieldBRequired, 
        outputConstructor = outputConstructor, outputToField = outputToField, outputToFieldIndex = outputToFieldIndex, 
        overrideOutputField = overrideOutputField), class = "gar_Filter.advancedDetails")
}

#' Filter.lowercaseDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the filter of the type LOWER.
#' 
#' @param field The \link{} object or list of objects
#' @param fieldIndex Field to use in the filter
#' @param field The Index of the custom dimension
#' 
#' @return Filter.lowercaseDetails object
#' 
#' @family Filter functions
#' @export
Filter.lowercaseDetails <- function(field = NULL, fieldIndex = NULL) {
    structure(list(field = field, fieldIndex = fieldIndex), class = "gar_Filter.lowercaseDetails")
}

#' Filter.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for this filter. Points to the account to which this filter belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the account to which this filter belongs
#' 
#' @return Filter.parentLink object
#' 
#' @family Filter functions
#' @export
Filter.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#account"), class = "gar_Filter.parentLink")
}

#' Filter.searchAndReplaceDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the filter of the type SEARCH_AND_REPLACE.
#' 
#' @param caseSensitive The \link{} object or list of objects
#' @param field Determines if the filter is case sensitive
#' @param fieldIndex Field to use in the filter
#' @param replaceString The Index of the custom dimension
#' @param searchString Term to replace the search term with
#' @param caseSensitive Term to search
#' 
#' @return Filter.searchAndReplaceDetails object
#' 
#' @family Filter functions
#' @export
Filter.searchAndReplaceDetails <- function(caseSensitive = NULL, field = NULL, fieldIndex = NULL, 
    replaceString = NULL, searchString = NULL) {
    structure(list(caseSensitive = caseSensitive, field = field, fieldIndex = fieldIndex, 
        replaceString = replaceString, searchString = searchString), class = "gar_Filter.searchAndReplaceDetails")
}

#' Filter.uppercaseDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the filter of the type UPPER.
#' 
#' @param field The \link{} object or list of objects
#' @param fieldIndex Field to use in the filter
#' @param field The Index of the custom dimension
#' 
#' @return Filter.uppercaseDetails object
#' 
#' @family Filter functions
#' @export
Filter.uppercaseDetails <- function(field = NULL, fieldIndex = NULL) {
    structure(list(field = field, fieldIndex = fieldIndex), class = "gar_Filter.uppercaseDetails")
}

#' FilterExpression Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics filter expression.
#' 
#' @param caseSensitive The \link{} object or list of objects
#' @param expressionValue Determines if the filter is case sensitive
#' @param field Filter expression value
#' @param fieldIndex Field to filter
#' @param matchType The Index of the custom dimension
#' @param caseSensitive Match type for this filter
#' 
#' @return FilterExpression object
#' 
#' @family FilterExpression functions
#' @export
FilterExpression <- function(caseSensitive = NULL, expressionValue = NULL, field = NULL, 
    fieldIndex = NULL, matchType = NULL) {
    structure(list(caseSensitive = caseSensitive, expressionValue = expressionValue, 
        field = field, fieldIndex = fieldIndex, kind = "analytics#filterExpression", 
        matchType = matchType), class = "gar_FilterExpression")
}

#' FilterRef Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a profile filter link.
#' 
#' @param href The \link{} object or list of objects
#' @param id Link for this filter
#' @param href Filter ID
#' 
#' @return FilterRef object
#' 
#' @family FilterRef functions
#' @export
FilterRef <- function(href = NULL, id = NULL) {
    structure(list(href = href, id = id, kind = "analytics#filterRef"), class = "gar_FilterRef")
}

#' Filters Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A filter collection lists filters created by users in an Analytics account. Each resource in the collection corresponds to a filter.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of filters
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this filter collection
#' @param startIndex Link to previous page for this filter collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return Filters object
#' 
#' @family Filters functions
#' @export
Filters <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#filters", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Filters")
}

#' GaData Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Analytics data for a given view (profile).
#' 
#' @param GaData.columnHeaders The \link{GaData.columnHeaders} object or list of objects
#' @param GaData.dataTable The \link{GaData.dataTable} object or list of objects
#' @param GaData.dataTable.cols The \link{GaData.dataTable.cols} object or list of objects
#' @param GaData.dataTable.rows The \link{GaData.dataTable.rows} object or list of objects
#' @param GaData.dataTable.rows.c The \link{GaData.dataTable.rows.c} object or list of objects
#' @param GaData.profileInfo The \link{GaData.profileInfo} object or list of objects
#' @param GaData.query The \link{GaData.query} object or list of objects
#' @param GaData.totalsForAllResults The \link{GaData.totalsForAllResults} object or list of objects
#' @param columnHeaders Column headers that list dimension names followed by the metric names
#' @param containsSampledData Determines if Analytics data contains samples
#' @param dataTable 
#' @param id Unique ID for this data response
#' @param itemsPerPage The maximum number of rows the response can contain, regardless of the actual number of rows returned
#' @param nextLink Link to next page for this Analytics data query
#' @param previousLink Link to previous page for this Analytics data query
#' @param profileInfo Information for the view (profile), for which the Analytics data was requested
#' @param query Analytics data request query parameters
#' @param rows Analytics data rows, where each row contains a list of dimension values followed by the metric values
#' @param sampleSize The number of samples used to calculate the result
#' @param sampleSpace Total size of the sample space from which the samples were selected
#' @param selfLink Link to this page
#' @param totalResults The total number of rows for the query, regardless of the number of rows in the response
#' @param totalsForAllResults Total values for the requested metrics over all the results, not just the results returned in this response
#' 
#' @return GaData object
#' 
#' @family GaData functions
#' @export
GaData <- function(GaData.columnHeaders = NULL, GaData.dataTable = NULL, GaData.dataTable.cols = NULL, 
    GaData.dataTable.rows = NULL, GaData.dataTable.rows.c = NULL, GaData.profileInfo = NULL, 
    GaData.query = NULL, GaData.totalsForAllResults = NULL, columnHeaders = NULL, 
    containsSampledData = NULL, dataTable = NULL, id = NULL, itemsPerPage = NULL, 
    nextLink = NULL, previousLink = NULL, profileInfo = NULL, query = NULL, rows = NULL, 
    sampleSize = NULL, sampleSpace = NULL, selfLink = NULL, totalResults = NULL, 
    totalsForAllResults = NULL) {
    structure(list(GaData.columnHeaders = GaData.columnHeaders, GaData.dataTable = GaData.dataTable, 
        GaData.dataTable.cols = GaData.dataTable.cols, GaData.dataTable.rows = GaData.dataTable.rows, 
        GaData.dataTable.rows.c = GaData.dataTable.rows.c, GaData.profileInfo = GaData.profileInfo, 
        GaData.query = GaData.query, GaData.totalsForAllResults = GaData.totalsForAllResults, 
        columnHeaders = columnHeaders, containsSampledData = containsSampledData, 
        dataTable = dataTable, id = id, itemsPerPage = itemsPerPage, kind = "analytics#gaData", 
        nextLink = nextLink, previousLink = previousLink, profileInfo = profileInfo, 
        query = query, rows = rows, sampleSize = sampleSize, sampleSpace = sampleSpace, 
        selfLink = selfLink, totalResults = totalResults, totalsForAllResults = totalsForAllResults), 
        class = "gar_GaData")
}

#' GaData.columnHeaders Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return GaData.columnHeaders object
#' 
#' @family GaData functions
#' @export
GaData.columnHeaders <- function() {
    list()
}

#' GaData.dataTable Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param GaData.dataTable.cols The \link{GaData.dataTable.cols} object or list of objects
#' @param GaData.dataTable.rows The \link{GaData.dataTable.rows} object or list of objects
#' @param GaData.dataTable.rows.c The \link{GaData.dataTable.rows.c} object or list of objects
#' @param cols The \link{GaData.dataTable.cols} object or list of objects
#' @param rows The \link{GaData.dataTable.rows} object or list of objects
#' 
#' @return GaData.dataTable object
#' 
#' @family GaData functions
#' @export
GaData.dataTable <- function(GaData.dataTable.cols = NULL, GaData.dataTable.rows = NULL, 
    GaData.dataTable.rows.c = NULL, cols = NULL, rows = NULL) {
    structure(list(GaData.dataTable.cols = GaData.dataTable.cols, GaData.dataTable.rows = GaData.dataTable.rows, 
        GaData.dataTable.rows.c = GaData.dataTable.rows.c, cols = cols, rows = rows), 
        class = "gar_GaData.dataTable")
}

#' GaData.dataTable.cols Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return GaData.dataTable.cols object
#' 
#' @family GaData functions
#' @export
GaData.dataTable.cols <- function() {
    list()
}

#' GaData.dataTable.rows Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param GaData.dataTable.rows.c The \link{GaData.dataTable.rows.c} object or list of objects
#' 
#' @return GaData.dataTable.rows object
#' 
#' @family GaData functions
#' @export
GaData.dataTable.rows <- function(GaData.dataTable.rows.c = NULL) {
    structure(list(GaData.dataTable.rows.c = GaData.dataTable.rows.c), class = "gar_GaData.dataTable.rows")
}

#' GaData.dataTable.rows.c Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return GaData.dataTable.rows.c object
#' 
#' @family GaData functions
#' @export
GaData.dataTable.rows.c <- function() {
    list()
}

#' GaData.profileInfo Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Information for the view (profile), for which the Analytics data was requested.
#' 
#' @param accountId The \link{} object or list of objects
#' @param internalWebPropertyId Account ID to which this view (profile) belongs
#' @param profileId Internal ID for the web property to which this view (profile) belongs
#' @param profileName View (Profile) ID
#' @param tableId View (Profile) name
#' @param webPropertyId Table ID for view (profile)
#' @param accountId Web Property ID to which this view (profile) belongs
#' 
#' @return GaData.profileInfo object
#' 
#' @family GaData functions
#' @export
GaData.profileInfo <- function(accountId = NULL, internalWebPropertyId = NULL, profileId = NULL, 
    profileName = NULL, tableId = NULL, webPropertyId = NULL) {
    structure(list(accountId = accountId, internalWebPropertyId = internalWebPropertyId, 
        profileId = profileId, profileName = profileName, tableId = tableId, webPropertyId = webPropertyId), 
        class = "gar_GaData.profileInfo")
}

#' GaData.query Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Analytics data request query parameters.
#' 
#' @param dimensions The \link{} object or list of objects
#' @param end-date List of analytics dimensions
#' @param filters End date
#' @param ids Comma-separated list of dimension or metric filters
#' @param max-results Unique table ID
#' @param metrics Maximum results per page
#' @param samplingLevel List of analytics metrics
#' @param segment Desired sampling level
#' @param sort Analytics advanced segment
#' @param start-date List of dimensions or metrics based on which Analytics data is sorted
#' @param start-index Start date
#' @param dimensions Start index
#' 
#' @return GaData.query object
#' 
#' @family GaData functions
#' @export
GaData.query <- function(dimensions = NULL, end.date = NULL, filters = NULL, ids = NULL, 
    max.results = NULL, metrics = NULL, samplingLevel = NULL, segment = NULL, sort = NULL, 
    start.date = NULL, start.index = NULL) {
    structure(list(dimensions = dimensions, `end-date` = end - date, filters = filters, 
        ids = ids, `max-results` = max - results, metrics = metrics, samplingLevel = samplingLevel, 
        segment = segment, sort = sort, `start-date` = start - date, `start-index` = start - 
            index), class = "gar_GaData.query")
}

#' GaData.totalsForAllResults Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Total values for the requested metrics over all the results, not just the results returned in this response. The order of the metric totals is same as the metric order specified in the request.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return GaData.totalsForAllResults object
#' 
#' @family GaData functions
#' @export
GaData.totalsForAllResults <- function() {
    list()
}

#' Goal Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics goal resource.
#' 
#' @param Goal.eventDetails The \link{Goal.eventDetails} object or list of objects
#' @param Goal.eventDetails.eventConditions The \link{Goal.eventDetails.eventConditions} object or list of objects
#' @param Goal.parentLink The \link{Goal.parentLink} object or list of objects
#' @param Goal.urlDestinationDetails The \link{Goal.urlDestinationDetails} object or list of objects
#' @param Goal.urlDestinationDetails.steps The \link{Goal.urlDestinationDetails.steps} object or list of objects
#' @param Goal.visitNumPagesDetails The \link{Goal.visitNumPagesDetails} object or list of objects
#' @param Goal.visitTimeOnSiteDetails The \link{Goal.visitTimeOnSiteDetails} object or list of objects
#' @param accountId Account ID to which this goal belongs
#' @param active Determines whether this goal is active
#' @param created Time this goal was created
#' @param eventDetails Details for the goal of the type EVENT
#' @param id Goal ID
#' @param internalWebPropertyId Internal ID for the web property to which this goal belongs
#' @param name Goal name
#' @param parentLink Parent link for a goal
#' @param profileId View (Profile) ID to which this goal belongs
#' @param selfLink Link for this goal
#' @param updated Time this goal was last modified
#' @param urlDestinationDetails Details for the goal of the type URL_DESTINATION
#' @param value Goal value
#' @param visitNumPagesDetails Details for the goal of the type VISIT_NUM_PAGES
#' @param visitTimeOnSiteDetails Details for the goal of the type VISIT_TIME_ON_SITE
#' @param webPropertyId Web property ID to which this goal belongs
#' 
#' @return Goal object
#' 
#' @family Goal functions
#' @export
Goal <- function(Goal.eventDetails = NULL, Goal.eventDetails.eventConditions = NULL, 
    Goal.parentLink = NULL, Goal.urlDestinationDetails = NULL, Goal.urlDestinationDetails.steps = NULL, 
    Goal.visitNumPagesDetails = NULL, Goal.visitTimeOnSiteDetails = NULL, accountId = NULL, 
    active = NULL, created = NULL, eventDetails = NULL, id = NULL, internalWebPropertyId = NULL, 
    name = NULL, parentLink = NULL, profileId = NULL, selfLink = NULL, updated = NULL, 
    urlDestinationDetails = NULL, value = NULL, visitNumPagesDetails = NULL, visitTimeOnSiteDetails = NULL, 
    webPropertyId = NULL) {
    structure(list(Goal.eventDetails = Goal.eventDetails, Goal.eventDetails.eventConditions = Goal.eventDetails.eventConditions, 
        Goal.parentLink = Goal.parentLink, Goal.urlDestinationDetails = Goal.urlDestinationDetails, 
        Goal.urlDestinationDetails.steps = Goal.urlDestinationDetails.steps, Goal.visitNumPagesDetails = Goal.visitNumPagesDetails, 
        Goal.visitTimeOnSiteDetails = Goal.visitTimeOnSiteDetails, accountId = accountId, 
        active = active, created = created, eventDetails = eventDetails, id = id, 
        internalWebPropertyId = internalWebPropertyId, kind = "analytics#goal", name = name, 
        parentLink = parentLink, profileId = profileId, selfLink = selfLink, type = type, 
        updated = updated, urlDestinationDetails = urlDestinationDetails, value = value, 
        visitNumPagesDetails = visitNumPagesDetails, visitTimeOnSiteDetails = visitTimeOnSiteDetails, 
        webPropertyId = webPropertyId), class = "gar_Goal")
}

#' Goal.eventDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the goal of the type EVENT.
#' 
#' @param Goal.eventDetails.eventConditions The \link{Goal.eventDetails.eventConditions} object or list of objects
#' @param eventConditions List of event conditions
#' @param useEventValue Determines if the event value should be used as the value for this goal
#' 
#' @return Goal.eventDetails object
#' 
#' @family Goal functions
#' @export
Goal.eventDetails <- function(Goal.eventDetails.eventConditions = NULL, eventConditions = NULL, 
    useEventValue = NULL) {
    structure(list(Goal.eventDetails.eventConditions = Goal.eventDetails.eventConditions, 
        eventConditions = eventConditions, useEventValue = useEventValue), class = "gar_Goal.eventDetails")
}

#' Goal.eventDetails.eventConditions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Goal.eventDetails.eventConditions object
#' 
#' @family Goal functions
#' @export
Goal.eventDetails.eventConditions <- function() {
    list()
}

#' Goal.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for a goal. Points to the view (profile) to which this goal belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the view (profile) to which this goal belongs
#' 
#' @return Goal.parentLink object
#' 
#' @family Goal functions
#' @export
Goal.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#profile"), class = "gar_Goal.parentLink")
}

#' Goal.urlDestinationDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the goal of the type URL_DESTINATION.
#' 
#' @param Goal.urlDestinationDetails.steps The \link{Goal.urlDestinationDetails.steps} object or list of objects
#' @param caseSensitive Determines if the goal URL must exactly match the capitalization of visited URLs
#' @param firstStepRequired Determines if the first step in this goal is required
#' @param matchType Match type for the goal URL
#' @param steps List of steps configured for this goal funnel
#' @param url URL for this goal
#' 
#' @return Goal.urlDestinationDetails object
#' 
#' @family Goal functions
#' @export
Goal.urlDestinationDetails <- function(Goal.urlDestinationDetails.steps = NULL, caseSensitive = NULL, 
    firstStepRequired = NULL, matchType = NULL, steps = NULL, url = NULL) {
    structure(list(Goal.urlDestinationDetails.steps = Goal.urlDestinationDetails.steps, 
        caseSensitive = caseSensitive, firstStepRequired = firstStepRequired, matchType = matchType, 
        steps = steps, url = url), class = "gar_Goal.urlDestinationDetails")
}

#' Goal.urlDestinationDetails.steps Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Goal.urlDestinationDetails.steps object
#' 
#' @family Goal functions
#' @export
Goal.urlDestinationDetails.steps <- function() {
    list()
}

#' Goal.visitNumPagesDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the goal of the type VISIT_NUM_PAGES.
#' 
#' @param comparisonType The \link{} object or list of objects
#' @param comparisonValue Type of comparison
#' @param comparisonType Value used for this comparison
#' 
#' @return Goal.visitNumPagesDetails object
#' 
#' @family Goal functions
#' @export
Goal.visitNumPagesDetails <- function(comparisonType = NULL, comparisonValue = NULL) {
    structure(list(comparisonType = comparisonType, comparisonValue = comparisonValue), 
        class = "gar_Goal.visitNumPagesDetails")
}

#' Goal.visitTimeOnSiteDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Details for the goal of the type VISIT_TIME_ON_SITE.
#' 
#' @param comparisonType The \link{} object or list of objects
#' @param comparisonValue Type of comparison
#' @param comparisonType Value used for this comparison
#' 
#' @return Goal.visitTimeOnSiteDetails object
#' 
#' @family Goal functions
#' @export
Goal.visitTimeOnSiteDetails <- function(comparisonType = NULL, comparisonValue = NULL) {
    structure(list(comparisonType = comparisonType, comparisonValue = comparisonValue), 
        class = "gar_Goal.visitTimeOnSiteDetails")
}

#' Goals Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A goal collection lists Analytics goals to which the user has access. Each view (profile) can have a set of goals. Each resource in the Goal collection corresponds to a single Analytics goal.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of goals
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this goal collection
#' @param startIndex Link to previous page for this goal collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of resources in the result
#' @param items Email ID of the authenticated user
#' 
#' @return Goals object
#' 
#' @family Goals functions
#' @export
Goals <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#goals", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Goals")
}

#' McfData Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Multi-Channel Funnels data for a given view (profile).
#' 
#' @param McfData.columnHeaders The \link{McfData.columnHeaders} object or list of objects
#' @param McfData.profileInfo The \link{McfData.profileInfo} object or list of objects
#' @param McfData.query The \link{McfData.query} object or list of objects
#' @param McfData.totalsForAllResults The \link{McfData.totalsForAllResults} object or list of objects
#' @param columnHeaders Column headers that list dimension names followed by the metric names
#' @param containsSampledData Determines if the Analytics data contains sampled data
#' @param id Unique ID for this data response
#' @param itemsPerPage The maximum number of rows the response can contain, regardless of the actual number of rows returned
#' @param nextLink Link to next page for this Analytics data query
#' @param previousLink Link to previous page for this Analytics data query
#' @param profileInfo Information for the view (profile), for which the Analytics data was requested
#' @param query Analytics data request query parameters
#' @param rows Analytics data rows, where each row contains a list of dimension values followed by the metric values
#' @param sampleSize The number of samples used to calculate the result
#' @param sampleSpace Total size of the sample space from which the samples were selected
#' @param selfLink Link to this page
#' @param totalResults The total number of rows for the query, regardless of the number of rows in the response
#' @param totalsForAllResults Total values for the requested metrics over all the results, not just the results returned in this response
#' 
#' @return McfData object
#' 
#' @family McfData functions
#' @export
McfData <- function(McfData.columnHeaders = NULL, McfData.profileInfo = NULL, McfData.query = NULL, 
    McfData.totalsForAllResults = NULL, columnHeaders = NULL, containsSampledData = NULL, 
    id = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, profileInfo = NULL, 
    query = NULL, rows = NULL, sampleSize = NULL, sampleSpace = NULL, selfLink = NULL, 
    totalResults = NULL, totalsForAllResults = NULL) {
    structure(list(McfData.columnHeaders = McfData.columnHeaders, McfData.profileInfo = McfData.profileInfo, 
        McfData.query = McfData.query, McfData.totalsForAllResults = McfData.totalsForAllResults, 
        columnHeaders = columnHeaders, containsSampledData = containsSampledData, 
        id = id, itemsPerPage = itemsPerPage, kind = "analytics#mcfData", nextLink = nextLink, 
        previousLink = previousLink, profileInfo = profileInfo, query = query, rows = rows, 
        sampleSize = sampleSize, sampleSpace = sampleSpace, selfLink = selfLink, 
        totalResults = totalResults, totalsForAllResults = totalsForAllResults), 
        class = "gar_McfData")
}

#' McfData.columnHeaders Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return McfData.columnHeaders object
#' 
#' @family McfData functions
#' @export
McfData.columnHeaders <- function() {
    list()
}

#' McfData.profileInfo Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Information for the view (profile), for which the Analytics data was requested.
#' 
#' @param accountId The \link{} object or list of objects
#' @param internalWebPropertyId Account ID to which this view (profile) belongs
#' @param profileId Internal ID for the web property to which this view (profile) belongs
#' @param profileName View (Profile) ID
#' @param tableId View (Profile) name
#' @param webPropertyId Table ID for view (profile)
#' @param accountId Web Property ID to which this view (profile) belongs
#' 
#' @return McfData.profileInfo object
#' 
#' @family McfData functions
#' @export
McfData.profileInfo <- function(accountId = NULL, internalWebPropertyId = NULL, profileId = NULL, 
    profileName = NULL, tableId = NULL, webPropertyId = NULL) {
    structure(list(accountId = accountId, internalWebPropertyId = internalWebPropertyId, 
        profileId = profileId, profileName = profileName, tableId = tableId, webPropertyId = webPropertyId), 
        class = "gar_McfData.profileInfo")
}

#' McfData.query Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Analytics data request query parameters.
#' 
#' @param dimensions The \link{} object or list of objects
#' @param end-date List of analytics dimensions
#' @param filters End date
#' @param ids Comma-separated list of dimension or metric filters
#' @param max-results Unique table ID
#' @param metrics Maximum results per page
#' @param samplingLevel List of analytics metrics
#' @param segment Desired sampling level
#' @param sort Analytics advanced segment
#' @param start-date List of dimensions or metrics based on which Analytics data is sorted
#' @param start-index Start date
#' @param dimensions Start index
#' 
#' @return McfData.query object
#' 
#' @family McfData functions
#' @export
McfData.query <- function(dimensions = NULL, end.date = NULL, filters = NULL, ids = NULL, 
    max.results = NULL, metrics = NULL, samplingLevel = NULL, segment = NULL, sort = NULL, 
    start.date = NULL, start.index = NULL) {
    structure(list(dimensions = dimensions, `end-date` = end - date, filters = filters, 
        ids = ids, `max-results` = max - results, metrics = metrics, samplingLevel = samplingLevel, 
        segment = segment, sort = sort, `start-date` = start - date, `start-index` = start - 
            index), class = "gar_McfData.query")
}

#' McfData.totalsForAllResults Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Total values for the requested metrics over all the results, not just the results returned in this response. The order of the metric totals is same as the metric order specified in the request.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return McfData.totalsForAllResults object
#' 
#' @family McfData functions
#' @export
McfData.totalsForAllResults <- function() {
    list()
}

#' Profile Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics view (profile).
#' 
#' @param Profile.childLink The \link{Profile.childLink} object or list of objects
#' @param Profile.parentLink The \link{Profile.parentLink} object or list of objects
#' @param Profile.permissions The \link{Profile.permissions} object or list of objects
#' @param accountId Account ID to which this view (profile) belongs
#' @param botFilteringEnabled Indicates whether bot filtering is enabled for this view (profile)
#' @param childLink Child link for this view (profile)
#' @param currency The currency type associated with this view (profile), defaults to USD
#' @param defaultPage Default page for this view (profile)
#' @param eCommerceTracking Indicates whether ecommerce tracking is enabled for this view (profile)
#' @param enhancedECommerceTracking Indicates whether enhanced ecommerce tracking is enabled for this view (profile)
#' @param excludeQueryParameters The query parameters that are excluded from this view (profile)
#' @param id View (Profile) ID
#' @param name Name of this view (profile)
#' @param parentLink Parent link for this view (profile)
#' @param permissions Permissions the user has for this view (profile)
#' @param siteSearchCategoryParameters Site search category parameters for this view (profile)
#' @param siteSearchQueryParameters The site search query parameters for this view (profile)
#' @param starred Indicates whether this view (profile) is starred or not
#' @param stripSiteSearchCategoryParameters Whether or not Analytics will strip search category parameters from the URLs in your reports
#' @param stripSiteSearchQueryParameters Whether or not Analytics will strip search query parameters from the URLs in your reports
#' @param timezone Time zone for which this view (profile) has been configured
#' @param websiteUrl Website URL for this view (profile)
#' 
#' @return Profile object
#' 
#' @family Profile functions
#' @export
Profile <- function(Profile.childLink = NULL, Profile.parentLink = NULL, Profile.permissions = NULL, 
    accountId = NULL, botFilteringEnabled = NULL, childLink = NULL, currency = NULL, 
    defaultPage = NULL, eCommerceTracking = NULL, enhancedECommerceTracking = NULL, 
    excludeQueryParameters = NULL, id = NULL, name = NULL, parentLink = NULL, permissions = NULL, 
    siteSearchCategoryParameters = NULL, siteSearchQueryParameters = NULL, starred = NULL, 
    stripSiteSearchCategoryParameters = NULL, stripSiteSearchQueryParameters = NULL, 
    timezone = NULL, websiteUrl = NULL) {
    structure(list(Profile.childLink = Profile.childLink, Profile.parentLink = Profile.parentLink, 
        Profile.permissions = Profile.permissions, accountId = accountId, botFilteringEnabled = botFilteringEnabled, 
        childLink = childLink, currency = currency, defaultPage = defaultPage, eCommerceTracking = eCommerceTracking, 
        enhancedECommerceTracking = enhancedECommerceTracking, excludeQueryParameters = excludeQueryParameters, 
        id = id, name = name, parentLink = parentLink, permissions = permissions, 
        siteSearchCategoryParameters = siteSearchCategoryParameters, siteSearchQueryParameters = siteSearchQueryParameters, 
        starred = starred, stripSiteSearchCategoryParameters = stripSiteSearchCategoryParameters, 
        stripSiteSearchQueryParameters = stripSiteSearchQueryParameters, timezone = timezone, 
        type = type, websiteUrl = websiteUrl), class = "gar_Profile")
}

#' Profile.childLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Child link for this view (profile). Points to the list of goals for this view (profile).
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the list of goals for this view (profile)
#' 
#' @return Profile.childLink object
#' 
#' @family Profile functions
#' @export
Profile.childLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#goals"), class = "gar_Profile.childLink")
}

#' Profile.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for this view (profile). Points to the web property to which this view (profile) belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the web property to which this view (profile) belongs
#' 
#' @return Profile.parentLink object
#' 
#' @family Profile functions
#' @export
Profile.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#webproperty"), class = "gar_Profile.parentLink")
}

#' Profile.permissions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Permissions the user has for this view (profile).
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Profile.permissions object
#' 
#' @family Profile functions
#' @export
Profile.permissions <- function() {
    list()
}

#' ProfileFilterLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics profile filter link.
#' 
#' @param filterRef The \link{} object or list of objects
#' @param id Filter for this link
#' @param profileRef Profile filter link ID
#' @param rank View (Profile) for this link
#' @param filterRef The rank of this profile filter link relative to the other filters linked to the same profile
#' 
#' @return ProfileFilterLink object
#' 
#' @family ProfileFilterLink functions
#' @export
ProfileFilterLink <- function(filterRef = NULL, id = NULL, profileRef = NULL, rank = NULL) {
    structure(list(filterRef = filterRef, id = id, profileRef = profileRef, rank = rank), 
        class = "gar_ProfileFilterLink")
}

#' ProfileFilterLinks Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A profile filter link collection lists profile filter links between profiles and filters. Each resource in the collection corresponds to a profile filter link.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of profile filter links
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this profile filter link collection
#' @param startIndex Link to previous page for this profile filter link collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return ProfileFilterLinks object
#' 
#' @family ProfileFilterLinks functions
#' @export
ProfileFilterLinks <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#profileFilterLinks", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_ProfileFilterLinks")
}

#' ProfileRef Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a linked view (profile).
#' 
#' @param accountId The \link{} object or list of objects
#' @param href Account ID to which this view (profile) belongs
#' @param id Link for this view (profile)
#' @param internalWebPropertyId View (Profile) ID
#' @param name Internal ID for the web property to which this view (profile) belongs
#' @param webPropertyId Name of this view (profile)
#' @param accountId Web property ID of the form UA-XXXXX-YY to which this view (profile) belongs
#' 
#' @return ProfileRef object
#' 
#' @family ProfileRef functions
#' @export
ProfileRef <- function(accountId = NULL, href = NULL, id = NULL, internalWebPropertyId = NULL, 
    name = NULL, webPropertyId = NULL) {
    structure(list(accountId = accountId, href = href, id = id, internalWebPropertyId = internalWebPropertyId, 
        kind = "analytics#profileRef", name = name, webPropertyId = webPropertyId), 
        class = "gar_ProfileRef")
}

#' ProfileSummary Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics ProfileSummary. ProfileSummary returns basic information (i.e., summary) for a profile.
#' 
#' @param id The \link{} object or list of objects
#' @param name View (profile) ID
#' @param starred View (profile) name
#' @param id Indicates whether this view (profile) is starred or not
#' 
#' @return ProfileSummary object
#' 
#' @family ProfileSummary functions
#' @export
ProfileSummary <- function(id = NULL, name = NULL, starred = NULL) {
    structure(list(id = id, kind = "analytics#profileSummary", name = name, starred = starred, 
        type = type), class = "gar_ProfileSummary")
}

#' Profiles Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A view (profile) collection lists Analytics views (profiles) to which the user has access. Each resource in the collection corresponds to a single Analytics view (profile).
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of views (profiles)
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this view (profile) collection
#' @param startIndex Link to previous page for this view (profile) collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return Profiles object
#' 
#' @family Profiles functions
#' @export
Profiles <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#profiles", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Profiles")
}

#' RealtimeData Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Real time data for a given view (profile).
#' 
#' @param RealtimeData.columnHeaders The \link{RealtimeData.columnHeaders} object or list of objects
#' @param RealtimeData.profileInfo The \link{RealtimeData.profileInfo} object or list of objects
#' @param RealtimeData.query The \link{RealtimeData.query} object or list of objects
#' @param RealtimeData.totalsForAllResults The \link{RealtimeData.totalsForAllResults} object or list of objects
#' @param columnHeaders Column headers that list dimension names followed by the metric names
#' @param id Unique ID for this data response
#' @param profileInfo Information for the view (profile), for which the real time data was requested
#' @param query Real time data request query parameters
#' @param rows Real time data rows, where each row contains a list of dimension values followed by the metric values
#' @param selfLink Link to this page
#' @param totalResults The total number of rows for the query, regardless of the number of rows in the response
#' @param totalsForAllResults Total values for the requested metrics over all the results, not just the results returned in this response
#' 
#' @return RealtimeData object
#' 
#' @family RealtimeData functions
#' @export
RealtimeData <- function(RealtimeData.columnHeaders = NULL, RealtimeData.profileInfo = NULL, 
    RealtimeData.query = NULL, RealtimeData.totalsForAllResults = NULL, columnHeaders = NULL, 
    id = NULL, profileInfo = NULL, query = NULL, rows = NULL, selfLink = NULL, totalResults = NULL, 
    totalsForAllResults = NULL) {
    structure(list(RealtimeData.columnHeaders = RealtimeData.columnHeaders, RealtimeData.profileInfo = RealtimeData.profileInfo, 
        RealtimeData.query = RealtimeData.query, RealtimeData.totalsForAllResults = RealtimeData.totalsForAllResults, 
        columnHeaders = columnHeaders, id = id, kind = "analytics#realtimeData", 
        profileInfo = profileInfo, query = query, rows = rows, selfLink = selfLink, 
        totalResults = totalResults, totalsForAllResults = totalsForAllResults), 
        class = "gar_RealtimeData")
}

#' RealtimeData.columnHeaders Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' No description
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return RealtimeData.columnHeaders object
#' 
#' @family RealtimeData functions
#' @export
RealtimeData.columnHeaders <- function() {
    list()
}

#' RealtimeData.profileInfo Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Information for the view (profile), for which the real time data was requested.
#' 
#' @param accountId The \link{} object or list of objects
#' @param internalWebPropertyId Account ID to which this view (profile) belongs
#' @param profileId Internal ID for the web property to which this view (profile) belongs
#' @param profileName View (Profile) ID
#' @param tableId View (Profile) name
#' @param webPropertyId Table ID for view (profile)
#' @param accountId Web Property ID to which this view (profile) belongs
#' 
#' @return RealtimeData.profileInfo object
#' 
#' @family RealtimeData functions
#' @export
RealtimeData.profileInfo <- function(accountId = NULL, internalWebPropertyId = NULL, 
    profileId = NULL, profileName = NULL, tableId = NULL, webPropertyId = NULL) {
    structure(list(accountId = accountId, internalWebPropertyId = internalWebPropertyId, 
        profileId = profileId, profileName = profileName, tableId = tableId, webPropertyId = webPropertyId), 
        class = "gar_RealtimeData.profileInfo")
}

#' RealtimeData.query Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Real time data request query parameters.
#' 
#' @param dimensions The \link{} object or list of objects
#' @param filters List of real time dimensions
#' @param ids Comma-separated list of dimension or metric filters
#' @param max-results Unique table ID
#' @param metrics Maximum results per page
#' @param sort List of real time metrics
#' @param dimensions List of dimensions or metrics based on which real time data is sorted
#' 
#' @return RealtimeData.query object
#' 
#' @family RealtimeData functions
#' @export
RealtimeData.query <- function(dimensions = NULL, filters = NULL, ids = NULL, max.results = NULL, 
    metrics = NULL, sort = NULL) {
    structure(list(dimensions = dimensions, filters = filters, ids = ids, `max-results` = max - 
        results, metrics = metrics, sort = sort), class = "gar_RealtimeData.query")
}

#' RealtimeData.totalsForAllResults Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Total values for the requested metrics over all the results, not just the results returned in this response. The order of the metric totals is same as the metric order specified in the request.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return RealtimeData.totalsForAllResults object
#' 
#' @family RealtimeData functions
#' @export
RealtimeData.totalsForAllResults <- function() {
    list()
}

#' Segment Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics segment.
#' 
#' @param created The \link{} object or list of objects
#' @param definition Time the segment was created
#' @param id Segment definition
#' @param name Segment ID
#' @param segmentId Segment name
#' @param selfLink Segment ID
#' @param updated Link for this segment
#' @param created Time the segment was last modified
#' 
#' @return Segment object
#' 
#' @family Segment functions
#' @export
Segment <- function(created = NULL, definition = NULL, id = NULL, name = NULL, segmentId = NULL, 
    selfLink = NULL, updated = NULL) {
    structure(list(created = created, definition = definition, id = id, kind = "analytics#segment", 
        name = name, segmentId = segmentId, selfLink = selfLink, type = type, updated = updated), 
        class = "gar_Segment")
}

#' Segments Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An segment collection lists Analytics segments that the user has access to. Each resource in the collection corresponds to a single Analytics segment.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of segments
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this segment collection
#' @param startIndex Link to previous page for this segment collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return Segments object
#' 
#' @family Segments functions
#' @export
Segments <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#segments", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Segments")
}

#' UnsampledReport Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for Analytics unsampled report resource.
#' 
#' @param UnsampledReport.cloudStorageDownloadDetails The \link{UnsampledReport.cloudStorageDownloadDetails} object or list of objects
#' @param UnsampledReport.driveDownloadDetails The \link{UnsampledReport.driveDownloadDetails} object or list of objects
#' @param accountId Account ID to which this unsampled report belongs
#' @param dimensions The dimensions for the unsampled report
#' @param end-date The end date for the unsampled report
#' @param filters The filters for the unsampled report
#' @param id Unsampled report ID
#' @param metrics The metrics for the unsampled report
#' @param profileId View (Profile) ID to which this unsampled report belongs
#' @param segment The segment for the unsampled report
#' @param start-date The start date for the unsampled report
#' @param title Title of the unsampled report
#' @param webPropertyId Web property ID to which this unsampled report belongs
#' 
#' @return UnsampledReport object
#' 
#' @family UnsampledReport functions
#' @export
UnsampledReport <- function(UnsampledReport.cloudStorageDownloadDetails = NULL, UnsampledReport.driveDownloadDetails = NULL, 
    accountId = NULL, dimensions = NULL, end.date = NULL, filters = NULL, id = NULL, 
    metrics = NULL, profileId = NULL, segment = NULL, start.date = NULL, title = NULL, 
    webPropertyId = NULL) {
    structure(list(UnsampledReport.cloudStorageDownloadDetails = UnsampledReport.cloudStorageDownloadDetails, 
        UnsampledReport.driveDownloadDetails = UnsampledReport.driveDownloadDetails, 
        accountId = accountId, dimensions = dimensions, `end-date` = end - date, 
        filters = filters, id = id, metrics = metrics, profileId = profileId, segment = segment, 
        `start-date` = start - date, title = title, webPropertyId = webPropertyId), 
        class = "gar_UnsampledReport")
}

#' UnsampledReport.cloudStorageDownloadDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Download details for a file stored in Google Cloud Storage.
#' 
#' @param bucketId The \link{} object or list of objects
#' @param objectId Id of the bucket the file object is stored in
#' @param bucketId Id of the file object containing the report data
#' 
#' @return UnsampledReport.cloudStorageDownloadDetails object
#' 
#' @family UnsampledReport functions
#' @export
UnsampledReport.cloudStorageDownloadDetails <- function(bucketId = NULL, objectId = NULL) {
    structure(list(bucketId = bucketId, objectId = objectId), class = "gar_UnsampledReport.cloudStorageDownloadDetails")
}

#' UnsampledReport.driveDownloadDetails Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Download details for a file stored in Google Drive.
#' 
#' @param documentId The \link{} object or list of objects
#' @param documentId Id of the document/file containing the report data
#' 
#' @return UnsampledReport.driveDownloadDetails object
#' 
#' @family UnsampledReport functions
#' @export
UnsampledReport.driveDownloadDetails <- function(documentId = NULL) {
    structure(list(documentId = documentId), class = "gar_UnsampledReport.driveDownloadDetails")
}

#' UnsampledReports Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' An unsampled report collection lists Analytics unsampled reports to which the user has access. Each view (profile) can have a set of unsampled reports. Each resource in the unsampled report collection corresponds to a single Analytics unsampled report.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of unsampled reports
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this unsampled report collection
#' @param startIndex Link to previous page for this unsampled report collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of resources in the result
#' @param items Email ID of the authenticated user
#' 
#' @return UnsampledReports object
#' 
#' @family UnsampledReports functions
#' @export
UnsampledReports <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, 
    previousLink = NULL, startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#unsampledReports", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_UnsampledReports")
}

#' Upload Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Metadata returned for an upload operation.
#' 
#' @param accountId The \link{} object or list of objects
#' @param customDataSourceId Account Id to which this upload belongs
#' @param errors Custom data source Id to which this data import belongs
#' @param id Data import errors collection
#' @param status A unique ID for this upload
#' @param accountId Upload status
#' 
#' @return Upload object
#' 
#' @family Upload functions
#' @export
Upload <- function(accountId = NULL, customDataSourceId = NULL, errors = NULL, id = NULL, 
    status = NULL) {
    structure(list(accountId = accountId, customDataSourceId = customDataSourceId, 
        errors = errors, id = id, kind = "analytics#upload", status = status), class = "gar_Upload")
}

#' Uploads Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Upload collection lists Analytics uploads to which the user has access. Each custom data source can have a set of uploads. Each resource in the upload collection corresponds to a single Analytics data upload.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of uploads
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this upload collection
#' @param startIndex Link to previous page for this upload collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param items The total number of results for the query, regardless of the number of resources in the result
#' 
#' @return Uploads object
#' 
#' @family Uploads functions
#' @export
Uploads <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#uploads", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults), class = "gar_Uploads")
}

#' UserRef Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a user reference.
#' 
#' @param email The \link{} object or list of objects
#' @param id Email ID of this user
#' @param email User ID
#' 
#' @return UserRef object
#' 
#' @family UserRef functions
#' @export
UserRef <- function(email = NULL, id = NULL) {
    structure(list(email = email, id = id, kind = "analytics#userRef"), class = "gar_UserRef")
}

#' WebPropertyRef Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for a web property reference.
#' 
#' @param accountId The \link{} object or list of objects
#' @param href Account ID to which this web property belongs
#' @param id Link for this web property
#' @param internalWebPropertyId Web property ID of the form UA-XXXXX-YY
#' @param name Internal ID for this web property
#' @param accountId Name of this web property
#' 
#' @return WebPropertyRef object
#' 
#' @family WebPropertyRef functions
#' @export
WebPropertyRef <- function(accountId = NULL, href = NULL, id = NULL, internalWebPropertyId = NULL, 
    name = NULL) {
    structure(list(accountId = accountId, href = href, id = id, internalWebPropertyId = internalWebPropertyId, 
        kind = "analytics#webPropertyRef", name = name), class = "gar_WebPropertyRef")
}

#' WebPropertySummary Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics WebPropertySummary. WebPropertySummary returns basic information (i.e., summary) for a web property.
#' 
#' @param id The \link{} object or list of objects
#' @param internalWebPropertyId Web property ID of the form UA-XXXXX-YY
#' @param level Internal ID for this web property
#' @param name Level for this web property
#' @param profiles Web property name
#' @param starred List of profiles under this web property
#' @param websiteUrl Indicates whether this web property is starred or not
#' @param id Website url for this web property
#' 
#' @return WebPropertySummary object
#' 
#' @family WebPropertySummary functions
#' @export
WebPropertySummary <- function(id = NULL, internalWebPropertyId = NULL, level = NULL, 
    name = NULL, profiles = NULL, starred = NULL, websiteUrl = NULL) {
    structure(list(id = id, internalWebPropertyId = internalWebPropertyId, kind = "analytics#webPropertySummary", 
        level = level, name = name, profiles = profiles, starred = starred, websiteUrl = websiteUrl), 
        class = "gar_WebPropertySummary")
}

#' Webproperties Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' A web property collection lists Analytics web properties to which the user has access. Each resource in the collection corresponds to a single Analytics web property.
#' 
#' @param items The \link{} object or list of objects
#' @param itemsPerPage A list of web properties
#' @param nextLink The maximum number of resources the response can contain, regardless of the actual number of resources returned
#' @param previousLink Link to next page for this web property collection
#' @param startIndex Link to previous page for this web property collection
#' @param totalResults The starting index of the resources, which is 1 by default or otherwise specified by the start-index query parameter
#' @param username The total number of results for the query, regardless of the number of results in the response
#' @param items Email ID of the authenticated user
#' 
#' @return Webproperties object
#' 
#' @family Webproperties functions
#' @export
Webproperties <- function(items = NULL, itemsPerPage = NULL, nextLink = NULL, previousLink = NULL, 
    startIndex = NULL, totalResults = NULL, username = NULL) {
    structure(list(items = items, itemsPerPage = itemsPerPage, kind = "analytics#webproperties", 
        nextLink = nextLink, previousLink = previousLink, startIndex = startIndex, 
        totalResults = totalResults, username = username), class = "gar_Webproperties")
}

#' Webproperty Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' JSON template for an Analytics web property.
#' 
#' @param Webproperty.childLink The \link{Webproperty.childLink} object or list of objects
#' @param Webproperty.parentLink The \link{Webproperty.parentLink} object or list of objects
#' @param Webproperty.permissions The \link{Webproperty.permissions} object or list of objects
#' @param accountId Account ID to which this web property belongs
#' @param childLink Child link for this web property
#' @param defaultProfileId Default view (profile) ID
#' @param id Web property ID of the form UA-XXXXX-YY
#' @param industryVertical The industry vertical/category selected for this web property
#' @param name Name of this web property
#' @param parentLink Parent link for this web property
#' @param permissions Permissions the user has for this web property
#' @param starred Indicates whether this web property is starred or not
#' @param websiteUrl Website url for this web property
#' 
#' @return Webproperty object
#' 
#' @family Webproperty functions
#' @export
Webproperty <- function(Webproperty.childLink = NULL, Webproperty.parentLink = NULL, 
    Webproperty.permissions = NULL, accountId = NULL, childLink = NULL, defaultProfileId = NULL, 
    id = NULL, industryVertical = NULL, name = NULL, parentLink = NULL, permissions = NULL, 
    starred = NULL, websiteUrl = NULL) {
    structure(list(Webproperty.childLink = Webproperty.childLink, Webproperty.parentLink = Webproperty.parentLink, 
        Webproperty.permissions = Webproperty.permissions, accountId = accountId, 
        childLink = childLink, defaultProfileId = defaultProfileId, id = id, industryVertical = industryVertical, 
        name = name, parentLink = parentLink, permissions = permissions, starred = starred, 
        websiteUrl = websiteUrl), class = "gar_Webproperty")
}

#' Webproperty.childLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Child link for this web property. Points to the list of views (profiles) for this web property.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the list of views (profiles) for this web property
#' 
#' @return Webproperty.childLink object
#' 
#' @family Webproperty functions
#' @export
Webproperty.childLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#profiles"), class = "gar_Webproperty.childLink")
}

#' Webproperty.parentLink Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Parent link for this web property. Points to the account to which this web property belongs.
#' 
#' @param href The \link{} object or list of objects
#' @param href Link to the account for this web property
#' 
#' @return Webproperty.parentLink object
#' 
#' @family Webproperty functions
#' @export
Webproperty.parentLink <- function(href = NULL) {
    structure(list(href = href, type = "analytics#account"), class = "gar_Webproperty.parentLink")
}


#' Webproperty.permissions Object
#' 
#' @details 
#' Autogenerated via \code{\link[googleAuthR]{gar_create_api_object}}
#' Permissions the user has for this web property.
#' 
#' @param  The \link{} object or list of objects
#' 
#' @return Webproperty.permissions object
#' 
#' @family Webproperty functions
#' @export


Webproperty.permissions <- function() {
    
    
    
    list()
    
}

