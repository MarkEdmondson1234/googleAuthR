...
f <- function(req){
  ...
  ## if the API response if of type "job"
  ## check if its done, or return FALSE (no cache) if it is not
  if(req$kind == "job"){
    return(req$status == "Done")
  }
  
  ## all other requests just check for 200 status codes
  req$status_code == 200
  
}

gar_cache_setup(invalid_func = f)  
```

Another example, if you also want to save the response to a file:

```r
gar_cache_setup(mcache = memoise::cache_filesystem("my_folder"),
                invalid_func = f)  
```
