...
This file location is set via `option("googleAuthR.httr_oauth_cache")`
...
If you call `googleAuthR::gar_auth()` then it will look for a cache token given by `option("googleAuthR.httr_oauth_cache")`- if not present it will trigger the OAuth2 flow.

If you call `googleAuthR::gar_auth("token_name")` then it will look for a cache token given by `"token_name"` and *set the `option("googleAuthR.httr_oauth_cache")` to `"token_name"`*. This means refreshes etc. will be to that token cache file. 
